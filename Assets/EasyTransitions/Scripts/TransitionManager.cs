...
namespace EasyTransition
{
    ...
    {        
        ...
        /// <param name="startDelay">The delay before the transition starts.</param>
        public void Transition(string sceneName, TransitionSettings transition, float startDelay,  bool hasSFX = false)
        {
            if (transition == null || runningTransition)
            {
                Debug.LogError("You have to assing a transition.");
            ...
            runningTransition = true;
            StartCoroutine(Timer(sceneName, startDelay, transition, hasSFX));
        }
        ...
        IEnumerator Timer(string sceneName, float startDelay, TransitionSettings transitionSettings, bool hasSFX)
        {
            yield return new WaitForSecondsRealtime(startDelay);
            if (hasSFX)
            {
                AudioManager.Instance.PlaySFX("GameplaySceneTransition");
            }
